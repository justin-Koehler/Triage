...
def test_reason_uses_model_text(client, monkeypatch):
    class Fake:
        name = "test"

        def complete_text(self, system, user):
            assert "papier" in user.lower()
            assert "Begründung" in system
            return "Anträge gehen verloren, der Stand ist unsichtbar."
    ...
    assert "verloren" in body["text"]
    assert "Urlaubsanträge auf Papier ersetzen" not in body["text"]


def test_reason_drops_copy_of_description(client, monkeypatch):
...
def test_reason_without_llm_returns_error(client):
    r = client.post(
        "/api/sessions/reason",
